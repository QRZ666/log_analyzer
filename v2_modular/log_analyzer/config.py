...
class ConfigManager:
    ...
    def load(self):
        ...
        self._config = AnalyzerConfig(
            ...
            rules_path=self._resolve_path(data.get("rules_path", "rules.json")),
            whitelist_path=self._resolve_path(data.get("whitelist_path", "whitelist.json")),
        )

    @staticmethod
    def _resolve_path(p: str) -> str:
        """相对路径基于配置文件所在目录解析（兼容 PyInstaller 打包后的 exe）"""
        path = Path(p)
        if path.is_absolute():
            return p
